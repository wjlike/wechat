...
public class FinanceBak {
    ...
    static {
    	//String soName = "WeWorkFinanceSdk.dll";
		String soName = "libWeWorkFinanceSdk_Java.so";
    	String soPath = FinanceBak.class.getResource("/").getPath();
		System.out.println("soPath=" + soPath);
		if (soName.endsWith(".dll") && soPath.startsWith("/")) {
...
//		String soPath = Finance.class.getResource("/libWeWorkFinanceSdk_Java.so").getPath();
//		//以上得到jar包里面的so文件，执行不了，换成jar包当前同级目录的
//		soPath = soPath.substring(0, soPath.indexOf(".jar!"));
//		soPath = soPath.substring(0, soPath.lastIndexOf("/") + 1) + "libWeWorkFinanceSdk_Java.so";
//		//如果路径以 "file:/" 开头，则去掉 "file:"
//		soPath = soPath.replace("file:/", "/");
//		System.load(soPath);
